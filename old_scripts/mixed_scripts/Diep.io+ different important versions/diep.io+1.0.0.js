// ==UserScript==
// @name         r!PsAw's Diep.io+ (BIG UPDATE)
...
let ui_scale = document.querySelector("#settings-screen > div > div:nth-child(1) > div > div:nth-child(1) > label > span")
function ui_scale_check(){
 if(ui_scale.innerHTML != "-" && ui_scale.innerHTML != null){
  if(ui_scale.innerHTML != "0.9x"){
   alert("please change your ui_scale to 0.9x in Menu -> Settings");
...
const observer = new MutationObserver((mutationsList, observer) => {
    let diep_console = document.getElementById("textInput");
    if (diep_console) {
        input.execute("net_predict_movement false");
        observer.disconnect(); // Stop observing after the element is found and script executed
    }
});
...
toggle_auto_r_btn.style.left = "35%";
...
function draw_minimap(){
   switch(gamemode){
       case "2tdm":
    for(let i = 0; i < minimap_elements.length; i++){
 
          //update their realTime position
          m_e_ctx_coords[i].startx = canvas.width-( minimap_elements[i].x * windowScaling() );
          m_e_ctx_coords[i].starty = canvas.height-( minimap_elements[i].y * windowScaling());
          m_e_ctx_coords[i].endx = m_e_ctx_coords[i].startx + minimap_elements[i].width*windowScaling();
          m_e_ctx_coords[i].endy = m_e_ctx_coords[i].starty + minimap_elements[i].height*windowScaling();
 
          //draw
          ctx.beginPath();
          ctx.rect(m_e_ctx_coords[i].startx, m_e_ctx_coords[i].starty, minimap_elements[i].width*windowScaling(), minimap_elements[i].height*windowScaling());
          ctx.lineWidth = "1";
          ctx.strokeStyle = minimap_elements[i].color;
          ctx.stroke();
    }
           break
       case "4tdm":
...
function draw_crx_events(){
    ...
    if(window.y_and_x){
      ...
      }else if(window.l_arrow && window.m_arrow){
    ...
    let gui = () => {
      if(script_boolean){
       if(ingamescreen.classList.contains("screen") && ingamescreen.classList.contains("active")){
        ...
        ctx_text("white", "DarkRed", 6, 1.5 + "em Ubuntu", "current Tick: " + currentTick, canvas.width/20 + 10, text_startingY + (textAdd*6));
        //ctx_text("white", "DarkRed", 6, 1.5 + "em Ubuntu", "realX: " + uwuX + "realY: " + uwuY + "newX: " + uwuX*windowScaling() + "newY: " + uwuY*windowScaling(), uwuX*2, uwuY*2);
         ...
         ctx_rect(uwuX*two, uwuY*two, 41, 36);
         /*
...
function AntiAfkTimeout(){
    window.requestAnimationFrame(AntiAfkTimeout);
    if(key_storage.length === 0){
        input.key_down(87);
        input.key_down(83);
        input.key_up(87);
        input.key_up(83);
...
function analyse_keys(){
    ...
    }else if(key_storage.includes("n")){
             //calculate_speed(m_s[0]);
